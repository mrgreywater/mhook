...
#endif
		Base |= (ULONG_PTR)Entry.BaseHi << 24;
		Base |= (ULONG_PTR)Entry.BaseMid << 16;
		Base |= Entry.BaseLow;
	}
	else
	{
		...
		{
			GATE_ENTRY *Gate = (GATE_ENTRY *)&Entry;
#ifdef _WIN64
			Base = ((ULONG_PTR)Gate->HighOffset64 << 32) | ((ULONG_PTR)Gate->HighOffset << 16) | Gate->LowOffset;
#else
